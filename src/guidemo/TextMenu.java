...
public class TextMenu extends JMenu {
	...
	public TextMenu(DrawPanel owner) {
		...
		final JMenuItem lh = new JMenuItem("Set Line Height...");
		lh.addActionListener( new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				double currentSize = panel.getTextItem().getLineHeightMultiplier();
				String s = JOptionPane.showInputDialog(panel, "Multiply the default line spacing by what amount?",currentSize);
				if (s != null && s.trim().length() > 0) {
					try {
